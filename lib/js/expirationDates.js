  ...
  if (companyId == 0) { // Grizzly | Kodiak
    ...
    switch (month1) {
      ...
      case 2:
        ...
        return first.getWeek() + ' ' + last.getWeek();
      case 3:
        first = new Date(year + '-03-01');
        last = new Date(year + '-03-31');

        return first.getWeek() + ' - ' + last.getWeek();
      case 4:
        first = new Date(year + '-04-01');
        last = new Date(year + '-04-30');

        return first.getWeek() + ' - ' + last.getWeek();
      case 5:
        first = new Date(year + '-05-01');
        last = new Date(year + '-05-31');

        return first.getWeek() + ' - ' + last.getWeek();
      case 6:
        first = new Date(year + '-06-01');
        last = new Date(year + '-06-30');

        return first.getWeek() + ' - ' + last.getWeek();
      case 7:
        first = new Date(year + '-07-01');
        last = new Date(year + '-07-31');

        return first.getWeek() + ' - ' + last.getWeek();
      case 8:
        first = new Date(year + '-08-01');
        last = new Date(year + '-08-31');

        return first.getWeek() + ' - ' + last.getWeek();
      case 9:
        first = new Date(year + '-09-01');
        last = new Date(year + '-09-30');

        return first.getWeek() + ' - ' + last.getWeek();
      case 10:
        first = new Date(year + '-10-01');
        last = new Date(year + '-10-31');

        return first.getWeek() + ' - ' + last.getWeek();
      case 11:
        first = new Date(year + '-11-01');
        last = new Date(year + '-11-30');

        return first.getWeek() + ' - ' + last.getWeek();
      case 12:
        first = new Date(year + '-12-01');
...
function expiration() {
  ...
  for (var i in elems) {
    elems[i].innerHTML = new Date().getFullYear();
  }
...
function dateCode() {
  document.getElementById('content').innerHTML = '';
  loadFile('content', 'templates/information.html').then(function(res) {
    var template = convertToElement(document.getElementById('monthTables').innerHTML);

    template.getElementsByClassName('jan')[0].innerHTML = getMonthCode(1, 0);
    template.getElementsByClassName('feb')[0].innerHTML = getMonthCode(2, 0);
    template.getElementsByClassName('mar')[0].innerHTML = getMonthCode(3, 0);
    template.getElementsByClassName('apr')[0].innerHTML = getMonthCode(4, 0);
    template.getElementsByClassName('may')[0].innerHTML = getMonthCode(5, 0);
    template.getElementsByClassName('jun')[0].innerHTML = getMonthCode(6, 0);
    template.getElementsByClassName('jul')[0].innerHTML = getMonthCode(7, 0);
    template.getElementsByClassName('aug')[0].innerHTML = getMonthCode(8, 0);
    template.getElementsByClassName('sep')[0].innerHTML = getMonthCode(9, 0);
    template.getElementsByClassName('oct')[0].innerHTML = getMonthCode(10, 0);
    template.getElementsByClassName('nov')[0].innerHTML = getMonthCode(11, 0);
    template.getElementsByClassName('dec')[0].innerHTML = getMonthCode(12, 0);

    var table = document.getElementById('grizzly-table');
    table.insertBefore(template.firstElementChild, table.childNodes[0]);
  });
}
